...
export async function adaptAllChapters(projectId: string): Promise<{ project_id: string; chapters_queued: number }> {
  const res = await fetch(`${BASE}/projects/${projectId}/adapt-all`, { method: "POST" });
